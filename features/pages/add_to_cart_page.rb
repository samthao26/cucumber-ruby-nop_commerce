class AddToCartPage
  ...
  def find_card_row(product_name)
  ...
  def click_quantity(product_name, direction)
    case direction
    when 'up'
      find_card_row(product_name).find(QUANTITY_UP).click
      when 'down'
        find_card_row(product_name).find(QUANTITY_DOWN).click
      end
      sleep(2)
      end

  def remove_item_from_cart_page(product_name)
    find_card_row(product_name).find(REMOVE_BUTTON).click
...
def verify_item_moved_out_shopping_cart_page(product_name)
  ...
  def verify_total_price(product_name)
    row = find_card_row(product_name)
