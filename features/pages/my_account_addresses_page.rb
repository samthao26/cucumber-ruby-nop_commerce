...
class MyAccountAddressesPage
  ...
  def select_state(state_name)
    return unless has_select?(STATE_SELECT_ADDRESS, with_options: [state_name], wait: 10)
  ...
  def verify_address(data)
    ...
    expect(address_item.find(EMAIL_VERIFY).text).to include(data['email'].to_s)
    expect(address_item.find(PHONE_VERIFY).text).to include(data['phone'].to_s)
    expect(address_item.find(FAX_VERIFY).text).to include(data['fax'].to_s)
    expect(address_item.find(COMPANY_VERIFY).text).to include(data['company'].to_s)
    expect(address_item.find(COUNTRY_VERIFY).text).to include(data['country'].to_s)
    if data['state'] && data['state'] != 'Other' && address_item.has_css?(STATE_VERIFY, wait: 2)
      expect(address_item.find(STATE_VERIFY).text).to include(data['state'].to_s)
    end
    expect(address_item.find(CITY_VERIFY).text).to include(data['city'].to_s)
    expect(address_item.find(ADDRESS_1_VERIFY).text).to include(data['address1'].to_s)
    if data['address2'] && !data['address2'].empty?
      expect(address_item.find(ADDRESS_2_VERIFY).text).to include(data['address2'])
    end
    expect(address_item.find(ZIP_CODE_VERIFY).text).to include(data['zip'].to_s)
  end
end
